...
import { Mic, MicOff, Sparkles, Plus, Edit3, Trash2, CheckCircle, Send } from 'lucide-react';
  ...
  return (
    <div className="max-w-2xl mx-auto">
      ...
      <header className="text-center mb-8 sm:mb-12">
        <div className="relative inline-block mb-6" role="presentation" aria-hidden="true">
          ...
          <div className="relative inline-flex items-center justify-center w-20 h-20 sm:w-24 sm:h-24 bg-gradient-to-r from-emerald-500 to-teal-600 rounded-3xl shadow-2xl transform hover:scale-110 transition-all duration-500 group">
            <Mic className="w-10 h-10 sm:w-12 sm:h-12 text-white group-hover:animate-bounce" aria-hidden="true" />
          </div>
        </div>
        ...
        <p className="text-lg sm:text-xl text-slate-600 font-medium px-4 leading-relaxed max-w-lg mx-auto">
          Turn your brilliant ideas into even more brilliant ideas with feedback from passionate creators who get it 🚀
        </p>
        
        {/* Progress Steps */}
        <nav aria-label="Form progress" className="flex justify-center items-center gap-2 mt-6 sm:mt-8">
          {[
            ...
            const isCompleted = step.active || (index === 0 && form.title && form.description && form.deposit);
            const isCurrent = index === 0 && !showQuestions || 
                             index === 1 && form.title && form.description && form.deposit && !showQuestions ||
            ...
            return (
              <div key={index} className="flex items-center">
                <div 
                  ...
                  role="img"
                  aria-label={`Step ${index + 1}: ${step.label} - ${isCompleted ? 'Completed' : isCurrent ? 'Current step' : 'Not started'}`}
                >
        ...
        <div className="text-xs sm:text-sm mt-2 font-medium text-center">
          {!showQuestions && !success && (
            <p className="text-purple-600">Share your vision and let the magic begin! ✨</p>
          )}
          {showQuestions && !success && (
            <p className="text-amber-600">Your AI questions are ready! Review and publish when you're happy 🤖</p>
          )}
          {success && (
            <p className="text-emerald-600">🎉 Amazing! Your project is live and ready for brilliant feedback! 🎉</p>
          )}
        </div>
      </header>
      ...
      <main className="relative">
        ...
        <form className="relative bg-white/90 backdrop-blur-xl rounded-3xl p-6 sm:p-8 shadow-2xl border border-emerald-200/50" aria-label="Create feedback request form">
          {success && (
            <div className="bg-gradient-to-r from-green-50 to-emerald-50 border-2 border-green-300 text-green-800 p-4 sm:p-6 rounded-2xl mb-6 sm:mb-8 shadow-lg animate-bounce">
              <div className="flex items-center gap-3 sm:gap-4">
                ...
                <div>
                  <h3 className="font-black text-base sm:text-lg">Amazing! Your request is live! 🎉</h3>
                  <p className="text-sm sm:text-base text-green-700 font-medium">People can now discover and respond to your project</p>
                </div>
              </div>
            </div>
          )}
        <div className="space-y-6 sm:space-y-8">
          <div className="group">
            ...
            <div className="relative">
              <Input
                ...
                placeholder="What amazing thing are you building? ✨"
                aria-describedby={errors.title ? "title-error" : undefined}
                aria-invalid={!!errors.title}
                required
              />
              {form.title && (
                <div className="absolute right-3 top-1/2 transform -translate-y-1/2 text-emerald-500 animate-bounce" aria-hidden="true">
                  ✨
                </div>
              )}
            </div>
            {errors.title && (
              <div 
                ...
                aria-live="polite"
              >
                <span className="text-base sm:text-lg animate-pulse" aria-hidden="true">⚠️</span>
                {errors.title}
              </div>
            )}
          </div>
          ...
          <div className="group">
            ...
            <div className="relative">
              <textarea
                ...
                placeholder="Tell us about your project! What makes it special? What feedback would make it even better? 🚀"
                aria-describedby={`description-helper ${errors.description ? 'description-error' : ''}`}
                aria-invalid={!!errors.description}
                required
              />
              {form.description && form.description.length > 50 && (
                <div className="absolute right-3 bottom-3 text-emerald-500 animate-pulse" aria-hidden="true">
                  💡
                </div>
              )}
            </div>
            {form.description && (
              <div id="description-helper" className="text-xs text-emerald-600 mt-1 flex items-center gap-1" aria-live="polite">
                <span className="w-2 h-2 bg-emerald-400 rounded-full animate-pulse" aria-hidden="true"></span>
                {form.description.length >= 100 ? `${form.description.length} characters - Great detail! 🎯` :
                 form.description.length >= 50 ? `${form.description.length} characters - Looking good! 👍` :
          ...
          <div className="group">
            ...
            <p className="text-xs text-slate-500 mb-2">Minimum $1 • Higher rewards attract more responses</p>
            <div className="relative">
              <Input
                id="reward-amount"
                name="deposit"
                ...
                aria-describedby={`deposit-helper ${errors.deposit ? 'deposit-error' : ''}`}
            ...
            {form.deposit && Number(form.deposit) > 0 && (
              <div id="deposit-helper" className="text-xs text-emerald-600 mt-1 flex items-center gap-1" aria-live="polite">
                <span className="w-2 h-2 bg-emerald-400 rounded-full animate-pulse" aria-hidden="true"></span>
                {Number(form.deposit) >= 50 ? "Wow! Premium feedback incoming! 🔥" : 
                 Number(form.deposit) >= 20 ? "Nice! This will attract great reviewers! 💫" :
                 "Perfect! Every project deserves thoughtful feedback! ✨"}
              </div>
            )}
            {errors.deposit && (
              <div 
        ...
        <fieldset className="mt-8 sm:mt-10 group">
          <legend className="block mb-3 sm:mb-4 font-black text-slate-800 text-lg sm:text-xl transform group-hover:scale-105 transition-transform duration-200 origin-left">🎤 Audio Brief (Optional)</legend>
          <div className="bg-blue-50 border-4 border-dashed border-blue-300 rounded-2xl sm:rounded-3xl p-6 sm:p-8 text-center hover:border-blue-400 transition-all duration-500 hover:bg-blue-100 hover:shadow-lg transform hover:scale-[1.02]" role="region" aria-labelledby="audio-section">
            <div className={`inline-flex items-center justify-center w-16 h-16 sm:w-20 sm:h-20 bg-blue-400 rounded-full mb-4 sm:mb-6 transform hover:scale-110 transition-transform duration-300 shadow-lg ${recording ? 'animate-pulse bg-red-500 shadow-red-200' : ''}`} aria-hidden="true">
              {recording ? (
            ...
            {recording && (
              <div className="mb-4 sm:mb-6" role="status" aria-live="polite">
                <div className="flex justify-center items-center gap-1 mb-2" aria-hidden="true">
                  <div className="w-2 h-2 bg-red-500 rounded-full animate-bounce"></div>
                  <div className="w-2 h-2 bg-red-500 rounded-full animate-bounce" style={{animationDelay: '0.1s'}}></div>
                  <div className="w-2 h-2 bg-red-500 rounded-full animate-bounce" style={{animationDelay: '0.2s'}}></div>
                </div>
                <p className="text-red-600 font-bold text-sm">Recording in progress...</p>
              </div>
            )}
            <div className="space-y-3 sm:space-y-4">
              {!recording && !audioUrl && (
                ...
                >
                  <Mic className="w-4 h-4 sm:w-5 sm:h-5 mr-2 sm:mr-3" aria-hidden="true" />
                  🎙️ Start Recording
                </Button>
              )}
              {recording && (
                ...
                >
                  <MicOff className="w-4 h-4 sm:w-5 sm:h-5 mr-2 sm:mr-3" aria-hidden="true" />
                  ⏹️ Stop Recording
                </Button>
              )}
              {audioUrl && (
                <div className="space-y-3 sm:space-y-4 animate-fade-in">
                  <div className="text-green-600 font-bold flex items-center justify-center gap-2 sm:gap-3 text-base sm:text-lg animate-bounce" role="status" aria-live="polite">
                    <CheckCircle className="w-5 h-5 sm:w-6 sm:h-6 animate-spin" aria-hidden="true" />
                    🎉 Recording saved!
                  </div>
                  <audio 
                    src={audioUrl} 
                  ...
                  />
                  <p className="text-blue-600 text-sm font-medium">Fantastic! Your voice adds that personal touch that makes feedback so much better! 🎤✨</p>
                </div>
              )}
            </div>
          </div>
        ...
        {/* Dynamic CTA Button */}
        {(() => {
          const hasRequiredFields = form.title && form.description && form.deposit;
          const isFormValid = form.title.length >= 5 && form.description.length >= 10 && Number(form.deposit) >= 1;
          ...
          return (
            ...
            >
              ...
              <div className="relative flex items-center justify-center">
                ...
                <Sparkles className="w-5 h-5 sm:w-6 sm:h-6 ml-2 sm:ml-3 group-hover:animate-spin" style={{animationDelay: '0.5s'}} />
              </div>
            </Button>
          );
        })()}
        {showQuestions && (
          <div className="mt-8 bg-gradient-to-br from-amber-50 to-orange-50 rounded-2xl p-6 border border-amber-200 animate-fade-in transform translate-y-0 opacity-100 transition-all duration-500">
            ...
            <div className="space-y-4 mb-6">
              {questions.map((q, i) => (
                <div key={i} className="bg-white/70 backdrop-blur-sm rounded-xl p-4 border border-white/50 shadow-sm hover:shadow-md transition-all duration-300 hover:transform hover:scale-[1.02] group animate-fade-in" style={{animationDelay: `${i * 0.1}s`}}>
                  {editingIndex === i ? (
          ...
          <div className="mt-8 space-y-6">
            ...
            <div className="bg-gradient-to-br from-emerald-50 to-teal-50 rounded-2xl p-6 border-2 border-emerald-200">
              ...
              <div className="space-y-3 text-sm">
                <div className="flex justify-between items-center">
                  ...
                  <span className="font-semibold text-slate-800 max-w-xs truncate">{form.title}</span>
                </div>
                <div className="flex justify-between items-center">
                  <span className="text-slate-600">Reward:</span>
                  <span className="font-bold text-emerald-600">${form.deposit}</span>
                </div>
                <div className="flex justify-between items-center">
                  <span className="text-slate-600">Questions:</span>
                  <span className="font-semibold text-slate-800">{questions.length} generated</span>
                </div>
                <div className="flex justify-between items-center">
                  <span className="text-slate-600">Audio:</span>
                  <span className="font-semibold text-slate-800">{audioUrl ? '✅ Included' : '⭕ Optional'}</span>
                </div>
              </div>
              <div className="mt-4 p-3 bg-white/50 rounded-xl border border-emerald-200/50">
                <p className="text-xs text-emerald-700 font-medium">
                  🌟 Ready to launch! Your project will be discovered by amazing creators who'll give you thoughtful feedback for ${form.deposit} each.
                </p>
              </div>
            </div>
            ...
            >
              ...
              <div className="relative flex items-center justify-center">
                ...
                🚀 Publish My Request 🚀
                <Send className="w-5 h-5 sm:w-6 sm:h-6 ml-2 sm:ml-3 group-hover:animate-bounce" style={{animationDelay: '0.2s'}} />
              </div>
            </Button>
          </div>
        )}
